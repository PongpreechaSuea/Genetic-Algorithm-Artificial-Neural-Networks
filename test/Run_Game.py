...
def run_game_with_ML(display, clock, weights,NN_S,snake_motion,display_width,display_height, MS , SC):
    ...
    for _ in range(test_games):
        ...
        for _ in range(steps_per_game):
            ...
            if collision_with_boundaries(snake_position[0],display_width,display_height) == 1 or collision_with_self(next_step.tolist(),
            ...
            snake_position, apple_position, score = play_game(snake_start, snake_position, apple_position,
                                                              button_direction, score, display, clock , snake_motion)

            
            if score > max_score:
                max_score = score
            SC , MS = Maxend_score(score,MS)
